...
static void deleteFixUp(RBTree *tree, RBNode *x,RBNode *parent) {
      //查找到兄弟节点，根据颜色进行调整
    RBNode *w;
    while (tree->root != x && (x == NULL || x->color == BLACK)) {  //调整双黑节点,x不存在或者存在但是为黑色,x不能为根节点
        if (parent->left == x) {  //x是父节点的左孩子
            w=parent->right;    //父节点的右孩子为兄弟节点w
            if (w->color==RED) {
                //情况1:x的兄弟是红色
                w->color=BLACK;    //将x的兄弟节点设置为黑色
                parent->color=RED; //将x的父节点设置为红色
                leftRotate(tree,parent); //对x父节点进行左旋
                w=parent->right;      //左旋后重新设置x对兄弟节点
            }
            //兄弟节点都是黑色
            if ((!w->left||w->left->color==BLACK)&&
               (!w->right||w->right->color==BLACK)) {
                //情况2:兄弟节点的子节点都是黑色
                w->color=RED; //将x对兄弟节点设置为红色
                x=parent;     //将向的父节点设置为新节点
                parent=x->parent;
            }else {
                //兄弟节点的子节点至少有一个红色
                if (!w->right||w->right->color==BLACK) {
                    //情况3:x的兄弟w是黑色，左孩子是红色的，右边是黑色的(异向红)
                    if (w->left) w->left->color=BLACK;  // 增加空指针保护
                    w->color=RED;
                    rightRotate(tree,w);
                    w=parent->right;
                }
                //情况4:x的兄弟w是黑色，x的兄弟右边孩子是红色(同向红)
                w->color=parent->color;
                parent->color=BLACK;
                if (w->right) w->right->color=BLACK;    // 增加空指针保护
                leftRotate(tree,parent);
                x=tree->root;
                break;
            }
            ...
            w=parent->left;     //父节点的左孩子为兄弟节点w
            if (w->color==RED) {
                //情况1:x的兄弟w是红色
                w->color=BLACK;
                parent->color=RED;
                // 修正：应旋转 parent，而不是 parent->left
                rightRotate(tree, parent);
                w=parent->left;
            }
            //兄弟节点的子节点都是黑色
            if ((!w->left||w->left->color==BLACK)&&
                ...
                //兄弟节点的子节点至少有一个红色
                //情况3:x的兄弟w是黑色，左孩子是黑色的，右边是红色的(异向红)
                if ((!w->left||w->left->color==BLACK)) {
                    if (w->right) w->right->color=BLACK;  // 增加空指针保护
                    w->color=RED;
                    leftRotate(tree,w);
...
static void deleteRBNode(RBTree* tree, RBNode *node) {
    ...
    if (y->parent==NULL) {//y的父节点为根
        tree->root=x;
    }else if (y->parent->left==y) {//y在父节点左边
        y->parent->left=x;
    }else {
        y->parent->right=x;
    }
